...
- highest_qualification:
  ...
  Highest education level with levels `A Levels`, `Degree`, `GCSE/CSE`,
  `GCSE/O Level`, `Higher/Sub Degree`, `No Qualification`, `ONC/BTEC`
  and `Other/Sub Degree`

- nationality:

  Nationality with levels `British`, `English`, `Irish`, `Scottish`,
  `Welsh`, `Other`, `Refused` and `Unknown`.

- ethnicity:

  Ethnicity with levels `Asian`, `Black`, `Chinese`, `Mixed`, `White`
  and `Refused` `Unknown`.
...
- region:
  ...
  Region with levels `London`, `Midlands & East Anglia`, `Scotland`,
...
#> Warning: Removed 1270 rows containing non-finite outside the scale range (`stat_bin()`).
