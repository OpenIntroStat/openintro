...
- theme:
  ...
  Visual theme ("default", "cerulean", "journal", "flatly", "readable",
...
- toc_float:
  ...
  `TRUE` to float the table of contents to the left of the main document
  content. Rather than `TRUE` you may also pass a list of options that
  control the behavior of the floating table of contents. See the
...
- code_folding:
  ...
  Specify `"none"` to display all code chunks (assuming they were knit
  with `echo = TRUE`). Specify `"hide"` to hide all R code chunks by
  default (users can show hidden code chunks either individually or
  document-wide). Specify `"show"` to show all R code chunks by default.
