`makeTube` <-
function(x, y, Z=2, R=1, col='#00000022', border='#00000000', type=c('lin', 'quad', 'robust'), stDev=c('constant', 'linear', 'other'), length.out=99, bw='default', plotTube=TRUE, addLine=TRUE, ...){
	n <- length(x)
	r <- range(x)
	R <- abs(R)
	R <- r + c(-R,R)*diff(r)
	...
	if(stDev %in% c('o', 'O', 'other', 'Other')){
		...
		Y <- Y[these]
		S <- S[these]
	} else if(stDev %in% c('L', 'l', 'linear', 'Linear')){
		if(bw[1] == 'default'){
			bw <- bw.nrd0(x)
		}
		S    <- rep(NA, length(X))
		...
		g <- lm(S ~ X, weights=sWts)
		S <- predict(g, list(X=X))
		these <- !is.na(Y) & !is.na(S) & (S > 0)
		X <- X[these]
		Y <- Y[these]
		S <- S[these]
	} else if(!(stDev %in% c('c', 'C', 'constant', 'Constant'))){
